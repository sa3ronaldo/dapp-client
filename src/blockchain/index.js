...
export const BlockchainProvider = ({children}) => {
    ...
    const [account, setAccount] = useState() // Vai ficar em uma VARIAVEL para Poder ser USADO em varios LUGARES
    ...
    const loadBlockchainData = async () => {
        ...
        setAccount(accounts[0]); // PEGA O primeiro que está em USO CONTA na METAMASK

        // Load smart contract
        const networkId = await web3.eth.net.getId() // PEGAR o Network ID da carteira
        const networkData = MemoryToken.networks[networkId]

        //Verificar se existe Conteudo no NETWORK
        if (networkData) {
            ...
            await geMyTokensFromBlockchain(_token, accounts[0]); // Pega o Tokens que tem na carteira atravez do Contrato (tokens)
